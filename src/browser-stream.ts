    ...
    session.openaiWs.send(
      JSON.stringify({
        ...
        item: {
          ...
          content: [{
            type: "input_text",
            text: `[System: set_volume already ran — now at level ${session.volumeLevel} of 3. Acknowledge briefly. Do not say you cannot control volume.]`,
          }],
        },
      })
    );
    ...
    session.openaiWs.send(
      JSON.stringify({
        ...
        item: {
          ...
          content: [{
            type: "input_text",
            text: `[System: play_music already ran — ${vibe} is playing in Agent Home. Acknowledge in one short sentence. Do not say you cannot play music or tell Lily to use another app.]`,
          }],
        },
      })
    );
      ...
      session.openaiWs.send(
        JSON.stringify({
          ...
          item: {
            ...
            content: [{
              type: "input_text",
              text: "[System: make_meme already ran — a visual meme with SVG illustration opened on CH-04. Describe what you see in one short sentence. NEVER say you cannot render images or tell Lily to use an external meme generator.]",
            }],
          },
        })
      );
  ...
  session.openaiWs.send(
    JSON.stringify({
      ...
      item: {
        ...
        content: [{
          type: "input_text",
          text: "[System: make_meme is generating a visual SVG meme on CH-04. Do NOT call open_text. Do NOT speak Top/Bottom caption text. Say one short line like 'Meme coming up on CH-04' or stay quiet.]",
        }],
      },
    })
  );
